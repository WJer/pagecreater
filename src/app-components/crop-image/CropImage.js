...
export default class CropImage extends Component {
  ...
  render() {
    ...
    if (!src) {
      return (
        <div className="app-util-cropimage-nullimg" onClick={this.props.onClickNullImage}>
          选择图片
        </div>
      );
    }
    return (
      <div className="app-util-cropimage">
        <div className="app-util-cropimage-imgbox">
        ...
        <div className="app-util-cropimage-btns">
          <a onClick={this.props.onChangeImage}>
            <i className="app-ico app-ico-icoreset" /> 换图
          </a>
          {delbtn ? (
            <a onClick={this.delImage}>
              <i className="app-ico app-ico-icodel" /> 删除
            </a>
          ) : null}
          <a onClick={this.resetCrop}>
            <i className="app-ico app-ico-icoinit" /> 重置
          </a>
          <a onClick={this.onCropOk}>
            <i className="app-ico app-ico-icocrop" /> 裁剪
          </a>
          <a className="app-img-down" href={src} rel="noopener noreferrer" target="_blank">
            {imageInfo ? <span style={{ fontSize: 12, color: imageInfo.size > 200 ? '#ff3636' : '#a5b6c8' }}>{`${imageInfo.size}kb`}</span> : '...'}
            <i className="app-ico app-ico-xiazai"></i>
          </a>
        </div>
      </div>
    );
