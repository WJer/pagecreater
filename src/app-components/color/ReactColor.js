...
export default class ReactColor extends Component {
  ...
  @bindSelf
  clickBank(e) {
    const $color = $(e.target).closest('.app-util-color');
  ...
  render() {
    ...
    return (
      <div className="app-util-color" id={this.id}>
        {children ? (
          <span className="app-util-color-inner" style={{ backgroundColor: color }} onClick={this.showPicker}>
            {children}
          </span>
        ) : (
          <span className="app-util-color-btn" style={{ backgroundColor: color }} onClick={this.showPicker} />
        )}
        {show ? (
          <div className="app-util-color-box" style={{ ...colorStyle }}>
            <SketchPicker
              presetColors={[
                '#D0021B',
                '#F5A623',
