 ...
 */
$(document).on('mousedown.uniqlist', '.app-uniqlist', function(e) {
  e.stopPropagation();
  var xd = e.pageX,
    yd = e.pageY,
    $this = $(this);
  ...
  // 点击其他区域，不拖动
  if (e.target.className === 'app-uniqlist') {
    return;
  }

  //处理 二次拖动clone的BUG
  if ($(e.target).closest('.app-uniq-clone')[0]) {
    return;
  }
  ...
  let cloneDom = () => {
    $li
      .addClass('app-uniq-start')
      .siblings('li')
      .removeClass('app-uniq-start');
    // 使用clone
    $clone = $($li.clone());
    $clone.addClass('app-uniq-clone').css({
      left: left,
      top: top,
      width: $li.width(),
      height: $li.height(),
