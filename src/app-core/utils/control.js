...
$.fn.control = function(setting) {
  ...
  set.scale = $('.app-js-canvas').transform('scale') || 1;
  let shtml = `<div class="app-control">
					{{rotate}}
					{{autosize}}
					{{fixedsize}}
  ...
  //如果没有旋转
  if (set.rotate) {
    shtml = shtml.replace('{{rotate}}', '<span class="app-control-rotate"></span>');
  } else {
  ...
  if (set.autosize) {
    shtml = shtml.replace(
      '{{autosize}}',
      `${set.setHeight ? `<span style="display: ${set.lockWideHigh ? 'none' : 'block'}" class="app-control-top"></span>` : ''}
					${set.setWidth ? `<span style="display: ${set.lockWideHigh ? 'none' : 'block'}" class="app-control-left"></span>` : ''}
					${set.setWidth ? `<span style="display: ${set.lockWideHigh ? 'none' : 'block'}" class="app-control-right"></span>` : ''}
					${set.setHeight ? `<span style="display: ${set.lockWideHigh ? 'none' : 'block'}" class="app-control-bottom"></span>` : ''}`
    );
  } else {
  ...
  let $controlDom = $this.find('.app-control');
  if ($controlDom[0]) {
    $controlDom.remove();
    $controlDom = null;
  }
  ...
  const rotateFun = function(e, _this) {
    // let $target = $(e.target);
    let $center = $(_this).find('.app-control-center');
    let $box = $(_this).parent();
    let center = {
  ...
  $(document)
    ...
    .on(`${drag.start}.control`, '.app-control', function(e) {
      ...
      set.scale = parseFloat($('.app-canvas-box').transform('scale')) || 1;
      switch (e.target.className) {
        case 'app-control':
          moveFun(e, this);
          break;
        case 'app-control-rotate':
          rotateFun(e, this);
          break;
        case 'app-control-top':
          resizeFun(e, this, 'top');
          break;
        case 'app-control-topleft':
          resizeFun(e, this, 'topleft');
          break;
        case 'app-control-topright':
          resizeFun(e, this, 'topright');
          break;
        case 'app-control-bottom':
          resizeFun(e, this, 'bottom');
          break;
        case 'app-control-bottomleft':
          resizeFun(e, this, 'bottomleft');
          break;
        case 'app-control-bottomright':
          resizeFun(e, this, 'bottomright');
          break;
        case 'app-control-left':
          resizeFun(e, this, 'left');
          break;
        case 'app-control-right':
