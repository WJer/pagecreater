...
$.contextMenu = function(setting) {
  ...
  if (!$('.app-contextmenu')[0]) {
      let tpl = `<div class="app-contextmenu" style="left: ${set.x}px; top: ${set.y}px;">
          <ul>
              ${set.vals
                  .map(elem => {
                      return `<li class="app-contextmenu-item" data-val="${elem.val}">${elem.name}</li>`;
                  })
                  .join('')}
          </ul>
      </div>`;
  ...
  $(document)
      ...
      .on('click.contextmenu', function(e) {
          let $context = $(e.target).closest('.app-contextmenu-item');
