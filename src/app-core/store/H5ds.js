...
import { cleanLayers } from '../app-app-preview/tools/cleanData';
...
export default class App {
  ...
  setControlLockWideHigh = lock => {
    ...
    const $control = $layer.find('.app-control');
    if ($control[0]) {
      $control.find('.app-control-top').css('display', lock ? 'none' : 'block');
      $control.find('.app-control-left').css('display', lock ? 'none' : 'block');
      $control.find('.app-control-bottom').css('display', lock ? 'none' : 'block');
      $control.find('.app-control-right').css('display', lock ? 'none' : 'block');
    }
  };
  ...
  getCanvasSize = () => {
    ...
    let canvasHeight = $(window).height(); // 画布高度减去时间轴和顶部高度
    let canvasWidth = $(window).width() - 350 - 140;
    if (canvasWidth < 0) {
      canvasWidth = 0;
    }
  ...
  addLayerGetCenterPosition = layerData => {
    ...
    const $realsize = $('.app-canvas-realsize');
  ...
  getPageLayerDom = index => {
    ...
    const $layers = $page
      .children('.app-swiper-layers-box')
      .children('.app-swiper-layers')
      .children('.layer'); // ('.app-swiper-layers > .layer');
