...
class EventList extends Component {
  ...
  render() {
    ...
    const cName = ['app-animatelist', 'app-eventlist'];
    if (show) {
      cName.push('app-animatelist-show');
    }
    return (
      <div className={cName.join(' ')}>
        <a onClick={this.closeList} className="app-basic-animatelist-close">
          <Icon type="right" theme="outlined" />
        </a>
        <Tabs size="small" defaultActiveKey="1">
          <TabPane tab="事件列表" key="1">
            <ul style={{ height }} className="app-global-clearfix">
              {eventset.map(elem => {
                const { name, eventId, icon } = elem;
                return (
                  <li
                    key={eventId}
                    className="app-event-item"
                    onClick={() => {
                      this.addEvent(elem);
                    }}
                  >
