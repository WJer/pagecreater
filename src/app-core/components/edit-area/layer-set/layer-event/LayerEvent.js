        ...
        <div className="app-event-list">
          <ul>
            {events.map(event => {
              ...
              return (
                <li onClick={() => this.clickHandler(eventId, id)} key={id} className="app-event">
                  <p>
                    <span>事件名称：</span>
                    {name}
                  </p>
          ...
          {this.props.h5ds.edata.eventListShow ? null : (
            <Button className="app-basic-animateset-addbtn" onClick={this.addEvent} type="primary" icon="plus">
              添加事件
            </Button>
          )}
        </div>
        <div className={'app-event-set ' + (!eventId ? 'app-event-hide' : 'app-event-show')}>
          {EventEditor ? (
