...
class AnimateSet extends Component {
  ...
  componentDidMount() {
    ...
    $(document).on('mousewheel.h5ds.AnimateSet', '.app-scroll-AnimateSet', e => {
      e.preventDefault();
  ...
  render() {
    ...
    this.layer.animate;
    return (
      <div className="app-basic-animateset">
        <div className="app-basic-animateset-list">
          <ul className="app-uniqlist" id="h5dsUniqAnimateSetList">
            {animate.map((elem, index) => {
              let cName = ['app-basic-animateset-item'];
              if (active === index) {
                cName.push('app-basic-animateset-active');
              }
              ...
              return (
                <li onClick={() => this.setActive(index)} className={cName.join(' ')} key={index + this.keys}>
                  <a onClick={e => this.delAnimate(e, index)} className="app-basic-animateset-btndel">
                  ...
                  <SetGrid span={2}>
                    <SetItem name="持续时间:">
                      <InputNumber
                        className="app-scroll-AnimateSet"
                        data-key="duration"
                        size="small"
                        step={0.1}
                        min={0}
          ...
          <Button className="app-basic-animateset-addbtn" onClick={this.addAnimate} type="primary" icon="plus">
            添加动画
          </Button>
        </div>
      </div>
