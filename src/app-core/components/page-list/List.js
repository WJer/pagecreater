...
import { Page } from '../../app-app-preview/pages';
    ...
    return (
      <ul className={cName.join(' ')} id="h5dsPageListId">
        {this.pages ? (
          this.pages.map((elem, index) => {
            ...
            return (
              <li
                ...
                className={'app-pagelist-page-item' + (selectNum === index ? ' app-pagelist-active' : '')}
              >
                <div className="app-pagelist-page-content" onClick={e => this.setPage(index, e)}>
                  {selectType === 'fixeds' && <div className="app-pagelist-fixeds-info">{this.setFixedsName(index)}</div>}
                  <div className="app-page-name">
                    <span>{index + 1}</span>
                    {elem.name}
                  </div>
                  {/* <div
                    className="app-page-min"
                    style={{
                ...
                <div className="app-pagelist-page-info">
                  {selectType !== 'fixeds' && [
                    <a key="1" onClick={e => this.copyPage(e, index)} className="app-pagelist-copy-page" title="复制页面">
                      <i className="app-ico app-ico-fuzhi" />
                    </a>,
                    <a key="2" onClick={e => e.nativeEvent.stopImmediatePropagation()} className="app-pagelist-del-page" title="删除页面">
                      <Popconfirm placement="rightBottom" title="是否删除该页面？" onConfirm={e => this.delPage(e, index)}>
                        <i className="app-ico app-ico-icodel" />
                      </Popconfirm>
                    </a>
                  ]}
                  {/* <a onClick={e => this.savePage(e, index)} className="app-pagelist-save-page" title="收藏页面">
                    <i className="app-ico app-ico-shoucang" />
                  </a> */}
                  <a onClick={e => this.addPage(e, index)} className="app-pagelist-add-page" title="添加页面">
                    <i className="app-ico app-ico-jia" />
                  </a>
                </div>
              </li>
            );
          ...
          <div className="app-pagelist-loading">页面载入中...</div>
        )}
      </ul>
    );
  }
