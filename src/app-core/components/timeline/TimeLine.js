...
class TimeLine extends Component {
  ...
  componentWillUnmount() {
    $('.app-timeline-slider').off('scroll');
  ...
  render() {
    ...
    return (
      <div className="app-timeline">
        <div className="app-timeline-btn" onMouseDown={this.props.setTimeLineHeight}>
          <i className="app-ico app-ico-tuodongcaozuo" />
        </div>
        {!this.props.show ? (
          <React.Fragment>
            <div className="app-timeline-header">
              <div className="app-timeline-left app-global-clearfix">
                <span className="app-timeline-item1">
                  <span className="app-timeline-play">
                    ...
                    <a onClick={this.banAnimate}>
                      <i className="app-ico app-ico-zanting1" />
                    </a>
                    <a onClick={this.playAnimate}>
                      <i className="app-ico app-ico-forward" />
                    </a>
                  </span>
                </span>
                <span className="app-timeline-item2">
                  <a>
                    <i className="app-ico app-ico-kejian" />
                  </a>
                  <a>
                    <i className="app-ico app-ico-suo" />
                  </a>
                </span>
              </div>
              <div className="app-timeline-right">
                <div
                  key={barKey}
                  className={'app-timeline-bar' + (banAnimate ? ' app-ban-animate' : '')}
                  style={{
                ...
                <ul className="app-timeline-scale app-js-timeline-scale" style={{ width: timeScale.length * eachLiWidth, transform: `translateX(${-left}px)` }}>
                  {timeScale.map(elem => {
                    return (
                      <li key={elem}>
                        {new Array(9).fill(1).map((d, i) => {
                          return <i key={i} className="app-timeline-dot" />;
                        })}
                        <span>{elem}s</span>
                      </li>
                    );
            ...
            <div className="app-timeline-body">
              <div className="app-timeline-left clearfix">
                <ul className="app-timeline-layers-left">
                  {layers.map((layer, index) => {
                    return (
                      <li onClick={e => this.setLayer(e, index)} className={selectLayer === index ? 'app-timeline-active' : ''} key={index}>
                        <span className="app-timeline-item1">{layer.name || layer.type}</span>
                        <span className="app-timeline-item2">
                          <a className={`app-timeline-view ${layer.set.hide ? '' : 'app-timeline-active'}`} />
                          <a className={`app-timeline-lock ${layer.set.lock ? 'app-timeline-active' : ''}`} />
                        </span>
                      </li>
                    );
                  })}
              ...
              <div className="app-timeline-right app-global-clearfix">
                <ul className="app-timeline-layers-right app-js-timeline-layers" style={{ width: timeScale.length * eachLiWidth + 10, transform: `translateX(${-left}px)` }}>
                  {layers.map((elem, index) => {
                    ...
                    if (elem.pid === 'h5ds_sound') {
                      ...
                      ReactDom = (
                        <li onClick={e => this.setLayer(e, index)} className={selectLayer === index ? 'app-timeline-active' : ''} data-keyid={elem.keyid} key={index}>
                          <span
                            style={{ left: eachLiWidth * delay, width: duration * eachLiWidth }}
                            className="app-timeline-timenode app-timeline-soundnode"
                          ...
                          {elem.animate.map((d, i) => {
                            ...
                            return (
                              <span
                                ...
                                className="app-timeline-timenode"
                                key={i}
            ...
            <div className="app-timeline-footer">
              <div className="app-timeline-left app-global-clearfix" />
              <div className="app-timeline-right app-global-clearfix">
                <div className="app-timeline-slider" onScroll={this.timeLineSliderScroll}>
                  <div style={{ width: timeScale.length * eachLiWidth }} className="app-timeline-slider-leng" />
                </div>
              </div>
            </div>
          </React.Fragment>
