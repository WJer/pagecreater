...
export function setGroupControl(getPageDom, indexs) {
  // 批量设置选中
  const dom = `<div class="app-control">
  ...
  // 先清除之前的
  pageDom.find('.app-control').remove();
...
export function dragGroupEvent(getLayers, getPageLayerDom, edata) {
  ...
  $('#h5dsCanvas')
    ...
    .on('mousedown.h5ds.group', '.app-control', function(ed) {
      // 如果选择的组为null，不拖动
...
export function initDragSelectGroupEvent(h5ds, callback) {
  ...
  $('#h5dsCanvas')
    ...
    .on('mousedown.h5ds.layerGroup', function(ed) {
      // 如果是选择的标线。不做处理
      if ($(ed.target).closest('.app-ruler')[0]) {
        return;
      }

      // 拖动导航器
      if ($(ed.target).closest('.app-canvas-navigator')[0]) {
        return;
      }

      // 拖动按钮修改页面长度和宽度
      if ($(ed.target).closest('.app-js-setpage')[0]) {
        return;
      }
      ...
      // 控制器的代码
      let controlBox = `<div class="app-control">
                    <span class="app-control-top"></span>
                    <span class="app-control-left"></span>
                    <span class="app-control-right"></span>
                    <span class="app-control-bottom"></span>
        ...
        .on('mousemove.h5ds.layerGroup', function(em) {
          ...
          arr.forEach(elem => {
            ...
            if (x > left && x < left + wid && y > top && y < top + hei) {
              ...
              const $control = elem.dom.find('.app-control');
              if (!$control[0]) {
                elem.dom.append(controlBox);
              }
            } else {
              elem.dom.find('.app-control').remove();
            }
          });
        })
