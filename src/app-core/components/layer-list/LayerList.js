...
class LayerList extends Component {
  ...
  setLayer(index, e) {
    ...
    if ($(e.currentTarget).find('.app-layerlist-locklayer-active')[0]) {
      message.warn('请先解锁图层');
      return;
    }
    ...
    if (e && e.ctrlKey) {
      ...
      transaction(() => {
        ...
        if (!edata.selectGroup) {
          edata.selectGroup = [];
          const $control = getPageDom().find('.app-control');
          // 判断之前是否有选中一个
          if ($control[0]) {
  ...
  render() {
    ...
    return (
      <div
        className="app-layerlist app-js-layerlist app-drag"
        data-dragset={`{"limit":true, "undrag":["#h5dsLayerList"]}`}
      >
        <h1 className="app-global-clearfix">
          <i className="app-ico app-ico-caidan" />
          <span>图层列表</span>
          <a onClick={this.showList}>
            <i className="app-ico app-ico-a1down" />
          </a>
        </h1>
        <ul id="h5dsLayerList" className="app-uniqlist" style={{ display: show ? 'block' : 'none' }}>
          {layers.length > 0 ? (
            layers.map((elem, index) => {
              const vCName = ['app-layerlist-showlayer'];
              const lCName = ['app-layerlist-locklayer'];
              if (elem.set.hide) {
                vCName.push('app-layerlist-hidelayer-active');
              }
              if (elem.set.lock) {
                lCName.push('app-layerlist-locklayer-active');
              }
              const classActive = ['app-js-layerlist-item'];

              // 如果是选择多个
              if (selectGroup) {
                if (selectGroup.indexOf(index) !== -1) {
                  classActive.push('app-layerlist-active');
                }
              ...
              return (
                ...
                >
                  <a onClick={e => this.viewLayer(e, elem)} className={vCName.join(' ')}>
                    <i className="app-ico app-ico-yanjing" />
                  </a>
                  ...
                  <a onClick={e => this.delLayer(e, index, elem)} className="app-layerlist-dellayer" title="删除图层">
                    <i className="app-ico app-ico-icodel" />
                  </a>
                  <a onClick={e => this.copyLayer(e, index)} className="app-layerlist-copylayer" title="复制图层">
            ...
            <li className="app-layerlist-null">
              <span>暂无图层</span>
            </li>
          )}
        </ul>
