...
class Editor extends Component {
  ...
  componentDidMount() {
    ...
    // 双击换图，可编辑
    $('#h5dsCanvas').on('dblclick.imgeditor', '.app-control', () => {
      this.changeImage();
    });
  }
  ...
  render() {
    ...
    return (
      <div className="app-edit-h5ds_img" key={cropKey}>
        <CropImage
          cropSet={crop}
...
// 图标
const icon = <i className="app-ico app-ico-beijing" />;
  ...
  util.imgLazy(url).then(_img => {
    if (_img) {
      transaction(() => {
        ...
        // 设置初始位置
        let { top, left } = $('.app-canvas-realsize').position();
