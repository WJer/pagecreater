...
import H5dsDrawer from '../../../../app-components/app-drawer';
...
export default class ShapeModal extends Component {
  ...
  drawShape(type) {
    ...
    $('.app-center').append(`<div class="app-shape-shapepanel"></div>`);
    this.hideModal();
  }
  ...
  addLayerData({ left, top, width, height, quadrant }) {
    ...
    const phoneScale = parseFloat($('.app-canvas-box').transform('scale')) || 1;
  ...
  drawTemporaryShape({ top, left, width, height, quadrant }) {
    if (!this.$temporary) {
      ...
      $('.app-shape-shapepanel').html(`<div class="app-shape-shapepanel-inner" style="width:0; height: 0;" id="${id}">${shtml}</div>`);
      this.$temporary = $(`#${id}`);
    } else {
      if (this.willDrawShape === 'line') {
  ...
  @bindSelf
  drawShapeEvent() {
    $(document).on('mousedown.h5ds.shapepanel', '.app-shape-shapepanel', ed => {
      const { left, top } = $('.app-center').offset();
      const size = {
        width: 0,
        height: 0,
        left: ed.pageX - left,
  ...
  render() {
    return (
      <H5dsDrawer className="app-shape-source" close={this.hideModal} title={null} show={this.state.visible}>
        <div className="app-shape-list">
          {shapes.map((d, i) => {
            return (
              <div className="app-shape-item" key={i} onClick={() => this.drawShape(d.type)}>
