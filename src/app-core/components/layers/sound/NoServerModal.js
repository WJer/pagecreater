  ...
  render() {
    ...
    return (
      <div className="app-noserver-modal">
        <div className="app-noserver-tips">未设置options.soundSourceModal参数，这里显示系统自带的音频管理器</div>
        <div>
        ...
        <div className="app-noserver-imgs">
          {this.state.imgList.map((d, i) => {
            return (
              <div key={i} className="app-noserver-item app-noserver-item-sound" onClick={() => this.addImg(d)}>
                <Icon
                  onClick={e => {
                    this.delImg(e, i);
                  }}
                  type="close"
                />
                <audio className="app-sound-controls" controls>
