import { Fixed, Page, Popup } from '../../app-app-preview/pages';
...
class CanvasView extends Component {
  ...
  initControlCallback() {
    ...
    // 先注销之前的控制器
    let $control = h5ds.getPageDom().find('.app-control');
    if ($control[0]) {
      $control.remove();
      $control = null;
    }
  ...
  initControlEvent() {
    ...
    window.pubSubEditor.subscribe('h5ds.destoryControl', data => {
      ...
      let $control = $('#h5dsCanvasApp').find('.app-control');
      if ($control[0]) {
        $control.remove();
        $control = null;
      }
  ...
  initLayerControl(index, e) {
    ...
    if (e && e.ctrlKey) {
      transaction(() => {
        ...
        if (!h5dsStore.edata.selectGroup) {
          ...
          const $control = h5dsStore.getPageDom().find('.app-control');
          // 判断之前是否有选中一个
          if ($control[0]) {
    ...
    if (h5dsStore.edata.selectGroup && util.isEmpty(h5dsStore.edata.selectLayer) && $(e.currentTarget).find('.app-control')[0]) {
      return;
    }
  ...
  render() {
    ...
    edata.keys;
    return (
      <div className="app-pages-app" key={edata.canvasAnimate}>
        {/* 弹窗页面 */}
        {edata.selectType === 'popups' && (
          <div className="app-fixed-view app-js-setsize" id="h5dsPageViewPopup" style={{ ...this.setPageStyle('popups') }}>
        ...
        {/* 上层浮动页面 */}
        <div className="app-fixed-view app-js-setsize" id="h5dsPageViewFixedUp" style={{ ...this.setPageStyle('fixedUp') }}>
        ...
        {/* 下层浮动页面 */}
        <div className="app-fixed-view app-js-setsize" id="h5dsPageViewFixedDown" style={{ ...this.setPageStyle('fixedDown') }}>
        ...
        <div className="app-page-view app-js-setsize" id="h5dsPageView" style={{ ...this.setPageStyle('pages') }}>
