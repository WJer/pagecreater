  ...
  guidesMousedown(e, elem) {
    ...
    $(document)
      ...
      .on('mouseup.h5ds.rulerline', () => {
        if (elem.type === 'row') {
          let left = $('.app-canvas-realsize').position().left;
          elem.value = (elem.style.left - left) / this.props.phoneScale;
        } else {
          // 设置相对位置
          let top = $('.app-canvas-realsize').position().top;
    ...
    return (
      <React.Fragment>
        {this.state.guides.map(elem => {
          return <li onMouseDown={e => this.guidesMousedown(e, elem)} className={'app-ruler-guides app-ruler-guides-' + elem.type} key={elem.id} style={{ ...elem.style }} />;
        })}
      </React.Fragment>
    );
  }
