...
export default class PageModule extends Component {
  ...
  newSwiper() {
    ...
    this.swiperInstance = new Swiper(`#${this.id}`, {
      ...
      effect,
      mousewheel: true,
      noSwipingClass: 'app-swiper-stop-swiping', // 锁定翻页
  ...
  render() {
    ...
    if (data.type === 'horizontal-phone') {
      transformStyle = {
        transform: `scale(${scale}) rotate(${direction === 'vertical' ? 90 : -90}deg)`,
    ...
    return (
      <div id={this.id} className="app-pages swiper-container" style={{ ...previewStyle }}>
        <div className="swiper-wrapper">
          {slides.map((index, i) => {
            ...
            className = [className];
            let boxClassName = ['app-pages-box', 'swiper-container'];
            const keyid = `keyid_` + page.keyid + `_${index}`;
            if (isLangPage) {
              boxClassName.push('app-swiper-page-scroll');
            }
            ...
            if (realIndex - index === 0) {
              ...
              className.push('swiper-slide-prev app-swiper-ban-animate');
            } else {
              className.push('swiper-slide-next app-swiper-ban-animate');
            }
            if (page.slider.lock) {
              className.push('app-swiper-stop-swiping');
            }
            ...
            console.log('realIndex - indexrealIndex - indexrealIndex - index', realIndex, index);
            ...
            return (
              ...
              >
                <div className={boxClassName.join(' ')} style={boxStyle}>
                  <div id={keyid} className={['app-swiper-pageinner', isLangPage ? 'swiper-container' : ''].join(' ')} style={{ ...innerStyle, ...transformStyle }}>
                    <Page
                      adsorbSet={{
                        scale,
                        adsorbTopBottom,
