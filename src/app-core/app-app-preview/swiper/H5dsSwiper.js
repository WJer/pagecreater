...
export default class H5dsSwiper extends Component {
  ...
  setPageStyle(page) {
     ...
     *
     * backgroundColor 设置到 app-swiper-box 上为了做最大化的兼容，
    ...
    let className = 'app-swiper-page swiper-slide';
    if (noSwiper) {
      className += ' app-swiper-page-lange';
    }
    if (page.className) {
  ...
  render() {
    ...
    const swiperClass = ['app-preview', `app-preview-${data.type}`];
    return (
      <div className={swiperClass.join(' ')} style={swiperStyle}>
        {data.type === 'ppt' ? (
          <div className="app-ppt-btn">
            <a className="app-ppt-prev" onClick={this.slidePrev}></a>
            <a className="app-ppt-next" onClick={this.slideNext}></a>
          </div>
        ) : null}
        <React.Fragment>
